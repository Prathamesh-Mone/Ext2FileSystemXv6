...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    if((addr = ip->addrs[bn]) == 0)
      ip->addrs[bn] = addr = ip->iops->balloc(ip->dev);
    return addr;
  }
  bn -= NDIRECT;
  ...
  if(bn < NINDIRECT){
    ...
    if((addr = ip->addrs[NDIRECT]) == 0)
      ip->addrs[NDIRECT] = addr = ip->iops->balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if((addr = a[bn]) == 0){
      a[bn] = addr = ip->iops->balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    return addr;
...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    if(ip->addrs[i]){
      ip->iops->bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  ...
  if(ip->addrs[NDIRECT]){
    ...
    for(j = 0; j < NINDIRECT; j++){
      if(a[j])
        ip->iops->bfree(ip->dev, a[j]);
    }
    brelse(bp);
    ip->iops->bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    bp = bread(ip->dev, ip->iops->bmap(ip, off/BSIZE));
